...
try:
    ...
    # NOTE: mlflow.pyfunc.load_model() does not always raise for a missing local path -
    # on some versions it silently returns None, so that must be checked explicitly.
    model = mlflow.pyfunc.load_model(MODEL_DIR)
    if model is None:
        raise RuntimeError(f"mlflow.pyfunc.load_model returned None for {MODEL_DIR}")
    # In Docker, everything is flattened into MODEL_DIR (model files, feature_columns.txt, etc.)
    ...
    try:
        ...
        # (mlruns/<exp_id>/models/m-<model_id>/artifacts/), not nested under the run's
        # own artifacts/ folder - so the model dir and the run's artifacts dir (where
        # feature_columns.txt/preprocessing.pkl live) must be located independently.
        import glob
        local_model_dirs = glob.glob("./mlruns/*/models/m-*/artifacts")
        local_feature_files = glob.glob("./mlruns/*/*/artifacts/feature_columns.txt")
        if local_model_dirs and local_feature_files:
            latest_model = max(local_model_dirs, key=os.path.getmtime)
            latest_feature_file = max(local_feature_files, key=os.path.getmtime)
            model = mlflow.pyfunc.load_model(latest_model)
            if model is None:
                raise RuntimeError(f"mlflow.pyfunc.load_model returned None for {latest_model}")
            MODEL_DIR = latest_model
            FEATURE_DIR = os.path.dirname(latest_feature_file)
